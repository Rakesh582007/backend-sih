...
def theta_deep_pct(soil_4: float, soil_5: float) -> float:
    """The real deep-soil-saturation formula: weighted mean of the 100cm
    and 150cm layers (the slip-plane band), weights 0.4/0.6.

    Public/pure on purpose — this is now the SINGLE source of truth for
    this formula across the whole merged system (the XGBoost models'
    deep_soil_moisture_pct feature, the false-positive filter's
    saturation gate, and this module's own landslide trigger matrix all
    call this same function, rather than each having their own copy
    that could silently drift). See schema.json's deep_soil_moisture_pct
    field, which documents this exact formula as the merged contract —
    it previously documented a different, speculative 30/60/100cm
    weighting that was never what this file actually computed.
    """
    return soil_4 * DEEP_SOIL_WEIGHTS["soil_4"] + soil_5 * DEEP_SOIL_WEIGHTS["soil_5"]


def _theta_deep_pct(reading: SensorReading) -> float:
    return theta_deep_pct(reading.soil_4, reading.soil_5)
